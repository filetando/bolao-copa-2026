...
export function PointsHistoryChart({ data }: Props) {
  ...
  // partida implícito) — um traço vertical discreto, sem texto, só pra indicar a troca.
  const marcosDeFase: string[] = []
  for (let i = 1; i < data.pontos.length; i++) {
    if (data.pontos[i].faseId !== data.pontos[i - 1].faseId) {
      marcosDeFase.push(data.pontos[i].dataHoraUtc)
    }
  }
